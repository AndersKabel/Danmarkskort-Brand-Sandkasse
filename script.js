...
if (buildingsOnly && buildingsOnly.length > 0) {
  ...
  for (const gid of grundIds) {
    try {
      const geoData = await fetchJordstykkeGeoJSON(gid);
...
if (Array.isArray(grundOnly) && grundOnly.length > 0) {
  ...
  grundOnly.forEach((g, idx) => {
    ...
    const pairs = summarizeGrund(g);
    html += renderKeyValueList(pairs);

    html += "<details><summary>Vis rå Grund-data</summary><pre>" + escapeHtml(JSON.stringify(g, null, 2)) + "</pre></details>";
    html += "</details>";
  });
...
// ----- EJENDOMSRELATION (pæn visning + rå data) -----
if (Array.isArray(ejendomsrelationOnly) && ejendomsrelationOnly.length > 0) {
  ...
  ejendomsrelationOnly.forEach((er, idx) => {
    ...
    const pairs = summarizeEjendomsrelation(er);
    html += renderKeyValueList(pairs);

    html += "<details><summary>Vis rå Ejendomsrelation-data</summary><pre>" + escapeHtml(JSON.stringify(er, null, 2)) + "</pre></details>";
    html += "</details>";
  });
